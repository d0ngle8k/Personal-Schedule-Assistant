...
def compare_results(expected, actual, test_input):
    ...
    # Support both test files that use 'event' and pipeline output 'event_name'
    exp_event_raw = expected.get("event") or expected.get("event_name")
    if exp_event_raw not in [None, "event_extracted", "event_with_typo"]:
        exp_event = normalize_text(exp_event_raw)
        # Pipeline historically returned 'event' but current pipeline uses 'event_name'
        act_event = normalize_text(actual.get("event_name") or actual.get("event"))
        ...
        if exp_event and act_event:
            ...
            if exp_event != act_event:
                # Allow if actual event is substring of expected or vice versa
