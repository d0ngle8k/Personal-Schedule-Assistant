...
# -*- coding: utf-8 -*-
"""
Test all JSON test case files and calculate overall accuracy
"""
...
def compare_results(expected, actual, field):
    ...
    field_mapping = {
        'event': 'event_name',
        'time_str': 'start_time',  # Compare time string (we'll check if time matches)
        'location': 'location',
    ...
    # Special handling for time_str - just check if time was extracted
    if field == 'time_str':
        # If expected time exists, check if actual start_time exists
...
def main():
    """Main test runner"""
    print("="*80)
    print("COMPREHENSIVE JSON TEST SUITE")
    print("="*80)
    ...
    report_path = test_dir / f"comprehensive_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
